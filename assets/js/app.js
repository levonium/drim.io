document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('[href="#contact"]').forEach((link) => {
    link.addEventListener('click', (e) => {
        ...
        <section id="contacts" aria-label="Get in touch"
          ...
          <div class="w-full h-full flex items-center justify-center">
              <div class="text-center">
                  ...
                  <span class="block mt-16 text-xl">Or you can start a chat on Telegram</span>

                  <a href="https://t.me/${username}" target="_blank" rel="noopener"
                    class="inline-block mt-8 text-lg sm:text-3xl font-bold text-brand-600 hover:text-brand-500 transition-colors duration-300">
                    <span class="sr-only">Telegram Chat Link</span>
                    <svg class="block size-24 mx-auto transform hover:scale-110 transition-transform duration-300" viewBox="0 0 240 240" aria-hidden="true">
                  ...
                  <button class="absolute bottom-4 left-1/2 -translate-x-1/2 cursor-pointer text-6xl text-slate-600 hover:text-slate-800 transition-colors duration-300 close-contact">
                    &times;
                  </button>
              </div>
          </div>
      ...
      // Then animate in after a small delay
      setTimeout(() => {
        contactsWrapper.querySelector('section')?.classList.remove('opacity-0', 'translate-y-4')
      }, 100)

      // Add event listener for close button
      const closeButton = contactsWrapper.querySelector('.close-contact')
      closeButton.addEventListener('click', () => {
        const section = contactsWrapper.querySelector('section')
